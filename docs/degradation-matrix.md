...
| D15 | Offscreen workroom spawn failure                      | spawn error / lease expiry              | interactive tiers continue in SW; AI job lanes fall through to the SW-local host and its heuristic rung (Principle 29) | respawn with capability-resolved reasons; workroom backoff 30s on loss | `src/roots/roots.test.ts` + `ops/tests/unit/infrastructure/ai-jobs-service.test.ts` (S6/S7/S8) |
...
## AI tier (pipeline shipped E8-T01; elevated providers remain provisions)
...
| D23 | Provider failure / breaker open | `ai-lanes` probe (breakers column) | next ladder rung, then forced-heuristic fallback; typed YIELDs (capability-absent / insufficient-evidence, E8-T03) pass through **without** breaker strikes; briefs prefer ABSENCE to guessing (Spec §5.13) | breaker cooldown (60s, half-open probe) | `docs/ai-provider-matrix.md` + S5 (`ai-jobs-service.test.ts`) + L1 (`ondevice-provider.test.ts`) + brief silence law A1–A6 (`ops/tests/unit/infrastructure/ai-briefs.test.ts`) + shadow G12 |

...
| D23 | Provider failure / breaker open | `ai-lanes` probe (breakers column) | next ladder rung, then forced-heuristic fallback; briefs prefer ABSENCE to guessing (Spec §5.13)            | breaker cooldown (60s, half-open probe) | `docs/ai-provider-matrix.md` + S5 (`ai-jobs-service.test.ts`) |

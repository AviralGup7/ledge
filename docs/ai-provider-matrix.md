...
| 2/3  | `builtin`    | `chrome-builtin-lm-v1` (LanguageModel, on-device) | capability-detected — 'downloadable' counts as absent (K1: no silent pulls) | opens at 3 strikes; yields never strike  | **shipped** (E8-T06: `mission-name` + `mission-summary` at the MED-floor 0.60 'suggested' stamp; **no briefs** — J2)             | U1–U10/L1–L3 (`ops/tests/unit/infrastructure/ai-builtin.test.ts`) + e2e degrade lane                                                       |
